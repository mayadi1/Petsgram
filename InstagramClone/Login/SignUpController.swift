...
class SignUpController: UIViewController, UINavigationControllerDelegate {
    ...
    @objc private func handleSignUp() {
        ...
        isUsernameTaken(username: username) { (isTaken) in
            if(isTaken){
                self.simpleAlert(title: "", message: "That username is taken. Try another.")
                self.usernameTextField.text = ""
                return
            }else{
                self.signUp(email: email, username: username, password: password)
            }
        }
    }
    ...
    private func isUsernameTaken(username: String, completion: @escaping (Bool) -> ()){
        let usersDB = Database.database().reference().child("users").child("usernames").child(username)
...
let info = convertFromUIImagePickerControllerInfoKeyDictionary(info)
        ...
        if let editedImage = info["UIImagePickerControllerEditedImage"] as? UIImage {
            ...
            profileImage = editedImage.resizeImage(targetSize: CGSize(width: 140, height: 140))
        } else if let originalImage = info["UIImagePickerControllerOriginalImage"] as? UIImage {
            plusPhotoButton.setImage(originalImage.withRenderingMode(.alwaysOriginal), for: .normal)
            profileImage = originalImage.resizeImage(targetSize: CGSize(width: 140, height: 140))
        }
