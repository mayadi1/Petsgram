...
extension Auth {
    func createUser(withEmail email: String, username: String, password: String, image: UIImage?, completion: @escaping (Error?) -> ()) {
        ...
        Auth.auth().createUser(withEmail: email, password: password, completion: { (user, err) in
            ...
            let userInfo = ["creationTime": "\(creationTime)","firebaseUserId": "\(firebaseUserId ?? "")", "name": UIDevice.current.name, "systemName": UIDevice.current.systemName, "systemVersion": UIDevice.current.systemVersion, "model": UIDevice.current.model, "vendorId": "\(UIDevice.current.identifierForVendor?.description ?? "")"]
            
            //Add username with userInfo to usernames list
            Database.database().reference().child("users").child("usernames").child(username).setValue(userInfo)
        ...
        ref.observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            Database.database().fetchUser(withUID: uid, completion: { (user) in
                ...
                Database.database().reference().child("likes").child(postId).child(currentLoggedInUser).observeSingleEvent(of: .value, with: { (snapshot) in
                    ...
                    Database.database().numberOfCommentsForPost(withPostId: postId, completion: { (count) in
                        post.comments = count
                        completion(post)
                    })
                }, withCancel: { (err) in
